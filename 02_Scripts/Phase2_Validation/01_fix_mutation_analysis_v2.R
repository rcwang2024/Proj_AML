...
# Fix mutation analysis - highest priority (simplified version)
...
# Load mutation data using read.delim (safer than fread)
cat("Loading mutation data...\n")
mutation_file <- "01_Data/BeatAML_Downloaded_Data/beataml_mutations.txt"

# Read just the first few lines to check structure
first_lines <- readLines(mutation_file, n = 5)
cat("First few lines of mutation file:\n")
print(first_lines)
cat("\n")

# Read the full data (may take a moment)
mutation_data <- read.delim(mutation_file, stringsAsFactors = FALSE, sep = "\t")

cat("Mutation data loaded:", nrow(mutation_data), "rows x", ncol(mutation_data), "columns\n")
...
if (is.null(id_col)) {
  cat("ERROR: Cannot identify sample ID column\n")
  cat("Please check column names above\n")
...
if (length(direct_match) == 0) {
  cat("⚠️ NO DIRECT MATCHES - Trying harmonization...\n\n")

  # Expression IDs likely have 'R' suffix (RNA)
  # Mutation IDs likely have 'D' suffix (DNA)
  # Try removing suffixes to match on base ID

  expr_clean <- str_replace(expr_sample_ids, "R$", "")
  mut_clean <- str_replace(mut_sample_ids, "D$", "")
  ...
  if (length(cleaned_match) > 0) {
    cat("✓ SUCCESS: Found", length(cleaned_match), "matches\n\n")
    ...
    cat("\n⚠️ STILL NO MATCHES\n")
    cat("Saving IDs for manual inspection...\n")
