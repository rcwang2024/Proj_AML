# TASK: Download and Prepare TCGA-LAML Data for External Validation
# Download expression and clinical data from TCGA-LAML

library(tidyverse)
library(TCGAbiolinks)

cat("=== TCGA-LAML DATA DOWNLOAD AND PREPARATION ===\n\n")

# ============================================================================
# 1. DOWNLOAD TCGA-LAML EXPRESSION DATA
# ============================================================================

cat("=== STEP 1: DOWNLOADING TCGA-LAML EXPRESSION DATA ===\n\n")

# Create output directory
dir.create("01_Data/TCGA_LAML", showWarnings = FALSE, recursive = TRUE)

# Query TCGA-LAML project for RNA-seq data
...
cat("Unique samples:", length(unique(query_tcga$results[[1]]$cases)), "\n\n")

# Download data (this may take 10-30 minutes depending on connection)
cat("Downloading TCGA-LAML data (this may take 10-30 minutes)...\n")
...
tryCatch({
  ...
  cat("Download completed at:", format(Sys.time(), "%H:%M:%S"), "\n\n")
  ...
  cat("This may be due to network issues or GDC server problems\n")
  cat("Please try again later or download manually from GDC Portal\n\n")
  quit(save = "no", status = 1)
})

# ============================================================================
# 2. EXTRACT AND NORMALIZE EXPRESSION DATA
# ============================================================================

cat("=== STEP 2: EXTRACTING AND NORMALIZING EXPRESSION ===\n\n")
...
# Normalize: TPM calculation (Transcripts Per Million)
# TPM = (counts / gene_length) * 1e6 / sum(counts / gene_length)
# For simplicity, we'll use DESeq2 normalization (similar to TMM)
...
# ============================================================================
# 3. DOWNLOAD AND PREPARE CLINICAL DATA
# ============================================================================

cat("=== STEP 3: DOWNLOADING CLINICAL DATA ===\n\n")
...
cat("=== STEP 4: MATCHING EXPRESSION AND CLINICAL DATA ===\n\n")
...
cat("TCGA-LAML DATA DOWNLOAD AND PREPARATION COMPLETE\n\n")
...
cat("### TCGA Download Task COMPLETE ###\n")
