# Complete remaining supplementary figures
library(magick)

cat("=== COMPLETING SUPPLEMENTARY FIGURES ===\n\n")
...
# Read PDFs - use first page only for multi-page PDFs
schoenfeld_img <- image_read_pdf("04_Figures/10_Model_Diagnostics/schoenfeld_residuals.pdf",
                                  pages = 1)

# DFBeta plots PDF might be multi-page, read first page only
...
cat("Created figures:\n")
figures <- list.files("04_Figures/13_Publication_Figures", pattern = "\\.(pdf|png)$")
for (fig in sort(figures)) {
  cat("  ✓", fig, "\n")
}

cat("\n### Supplementary Figures Task COMPLETE ###\n")
