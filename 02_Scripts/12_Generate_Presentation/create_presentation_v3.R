#!/usr/bin/env Rscript
# ==============================================================================
# BeatAML Project - Generate PowerPoint Presentation (Version 3)
# ==============================================================================
# Creates a presentation with key findings and PNG figures
# Date: 2025-10-09
# ==============================================================================

# Install required packages if needed
packages_needed <- c("officer", "pdftools")
...
for (name in names(figures_to_convert)) {
  ...
  if (file.exists(pdf_file)) {
    ...
    tryCatch({
      ...
      pdf_render_page(pdf_file, page = 1, dpi = 300, numeric = FALSE, antialias = TRUE) -> img_data
      png::writePNG(img_data, target = png_file)
...
if (!is.null(png_files$pca_before) && file.exists(png_files$pca_before)) {
  pres <- ph_with(pres,
...
if (!is.null(png_files$pca_after) && file.exists(png_files$pca_after)) {
  pres <- ph_with(pres,
...
if (!is.null(png_files$consensus) && file.exists(png_files$consensus)) {
  pres <- ph_with(pres,
                  external_img(png_files$consensus),
                  location = ph_location(left = 0.5, top = 1.0, width = 9, height = 6.2),
                  use_loc_size = TRUE)
} else {
  pres <- ph_with(pres, value = "[Figure: Consensus clustering matrices for k=2 to k=10]",
                  location = ph_location(left = 2, top = 3.5, width = 6, height = 1))
}
...
if (!is.null(png_files$pathway) && file.exists(png_files$pathway)) {
  pres <- ph_with(pres,
                  external_img(png_files$pathway),
                  location = ph_location(left = 1.0, top = 1.0, width = 8, height = 6.2),
                  use_loc_size = TRUE)
} else {
  pres <- ph_with(pres, value = "[Figure: Pathway heatmap showing complementary patterns]",
                  location = ph_location(left = 2, top = 3.5, width = 6, height = 1))
}
...
if (!is.null(png_files$km) && file.exists(png_files$km)) {
  pres <- ph_with(pres,
                  external_img(png_files$km),
                  location = ph_location(left = 1.0, top = 1.0, width = 8, height = 6.2),
                  use_loc_size = TRUE)
} else {
  pres <- ph_with(pres, value = "[Figure: Kaplan-Meier survival curves, p=0.00155]",
                  location = ph_location(left = 2, top = 3.5, width = 6, height = 1))
}
...
if (!is.null(png_files$drug) && file.exists(png_files$drug)) {
  pres <- ph_with(pres,
                  external_img(png_files$drug),
                  location = ph_location(left = 1.0, top = 1.0, width = 8, height = 6.2),
                  use_loc_size = TRUE)
} else {
  pres <- ph_with(pres, value = "[Figure: Drug sensitivity heatmap, 82 differential drugs]",
                  location = ph_location(left = 2, top = 3.5, width = 6, height = 1))
}
...
# Save presentation
# ------------------------------------------------------------------------------
output_file <- "04_Figures/BeatAML_Results_Presentation.pptx"
...
if (length(png_files) > 0) {
  cat(sprintf("PNG figures created in: 04_Figures/temp_png/ (%d files)\n\n", sum(sapply(png_files, function(x) !is.null(x)))))
}

cat("NOTE: If figures don't appear in PowerPoint, manually insert PNG files from:\n")
cat("      04_Figures/temp_png/\n\n")
