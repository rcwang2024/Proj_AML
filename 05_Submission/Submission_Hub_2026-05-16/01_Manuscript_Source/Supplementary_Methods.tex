...
\title{\Large\textbf{Supplementary Appendix: Transcriptomic Lineage Partitioning Resolves Venetoclax Resistance in Acute Myeloid Leukemia}}
\author{
...
\end{enumerate}
RNA-sequencing count data were normalized to counts per million (CPM) values, and low-expression genes (CPM $< 1$ in $> 90\%$ of samples) were filtered out. Expression values were $\log_2(CPM + 1)$ transformed. Technical batch effects associated with sequencing run waves were mitigated using the empirical Bayes framework implemented in the \texttt{ComBat} package (version 3.46.0) in R \cite{johnson2007combat}. Principal component analysis confirmed complete removal of batch-associated technical variation while preserving biological signal (\textbf{Supplementary Figure S21}).

\subsection{External Validation Cohorts}
\paragraph{TCGA-LAML Cohort:} We obtained matched RNA-seq count data and clinical annotations for 151 adult de novo AML patients from The Cancer Genome Atlas (TCGA-LAML) database through the Genomic Data Commons (GDC) portal \cite{cancer2013tcga}. Expression values were batch-corrected and normalized identically to the discovery cohort.
\paragraph{TARGET-AML Cohort:} To evaluate age-specific boundary effects, transcriptomic and clinical data for 1,713 pediatric AML patients (age 0--20 years) were acquired from the Therapeutically Applicable Research to Generate Effective Treatments (TARGET) initiative \cite{bolouri2018target}.
\paragraph{BeatAML 2.0 Validation Cohort:} An independent validation dataset comprising transcriptomic and functional profiling for 367 adult AML patients (representing Wave 3 and Wave 4 sequencing releases from the Bottomly et al., Cancer Cell 2022 dataset \cite{bottomly2022}) was utilized. These samples had no overlap with the discovery BeatAML cohort.
...
Unsupervised transcriptomic subtyping was performed using the \texttt{ConsensusClusterPlus} package (version 1.62.0) in R \cite{wilkerson2010consensuscluster}. We selected the top 5,000 most variable genes by median absolute deviation (MAD) from the batch-corrected, $\log_2$-transformed expression matrix. Consensus clustering was executed with the following parameters:
\begin{itemize}
...
\begin{enumerate}
    \item \textbf{Data Partitioning:} The 450-sample expression cohort was randomly partitioned into a 70\% training set ($n=315$) and a 30\% held-out internal validation set ($n=135$).
    ...
    \item \textbf{Performance Validation:} The classifier achieved $95.6\%$ classification accuracy, $92.2\%$ sensitivity, $97.6\%$ specificity, and a Receiver Operating Characteristic Area Under the Curve (ROC-AUC) of $0.994$ on this independent internal test split, and was subsequently applied to all external cohorts (classification statistics, proportions, and mean posterior probability mapping confidences across cohorts are detailed in \textbf{Supplementary Table~\ref{tab:tabS10}}).
\end{enumerate}
...
\subsection*{Table S1. Baseline Clinical and Molecular Characteristics of Molecular Subtypes (N=320)}
...
\subsection*{Table S6. Full Multivariate Cox Proportional Hazards Regression Analysis on Overall Survival (N=320)}
...
\subsection*{Table S8. Cluster 1 Salvage Therapy Candidates with Significant Ex Vivo Sensitivity Enrichment}
\hypertarget{tab:tabS8}{}
\label{tab:tabS8}
\begin{itemize}
    \item \textbf{Excel Workbook Sheet:} \texttt{Table S8}
    \item \textbf{Content:} Prioritized targeted therapy candidates demonstrating ex vivo sensitivity enrichment in resistant Cluster 1 patients.
    \item \textbf{Notes:} All compounds are targeted therapies. P-values: two-sided Wilcoxon rank-sum. Priority Candidate = pre-specified top salvage candidates for Cluster 1.
\end{itemize}

\subsection*{Table S9. Full Patient-Level Venetoclax Response Score (VRS) and Clinical Tier Assignments (N=320)}
...
\begin{itemize}
    ...
    \item \textbf{Notes:} VRS: Venetoclax Response Score (0-100). Tiers: Low ($<39.4$), Medium ($39.4$--$70.5$), High ($>70.5$).
\end{itemize}
...
Single-Cell RNA-Seq Reveals AML Hierarchies and Leukemic Stem Cell Characteristics.
...
\section{Extended Supplementary Methods and Protocols}

\subsection{Detailed Cohort Processing and Multi-Omics Integration}
BeatAML data (phs001657.v1.p1) were downloaded from dbGaP. Expression profiles were generated via RNA-sequencing of mononuclear cells from bone marrow and peripheral blood aspirates. Sequence reads were aligned to GRCh38, quantified as TPM, and log$_2$-transformed. Batch effects across sequencing centers (centerID) were corrected using ComBat empirical Bayes formulation \cite{johnson2007combat}. The 320-patient "Gold Standard" multi-omics cohort represents primary aspirates with complete log$_2$ expression, targeted variant calls for all 11 driver genes, ex vivo drug profiling across 155 compounds, and complete clinical overall survival data.

\subsection{Consensus Clustering Formulation and Stability Analysis}
Unsupervised consensus clustering was executed using ConsensusClusterPlus \cite{wilkerson2010consensuscluster} over 1,000 iterations with 80\% sample resampling, Euclidean distance metric, and Ward.D2 agglomerative linkage. Top variance genes were selected using Median Absolute Deviation (MAD). Consensus stability metrics included CDF slope area change, cluster consensus scores, and average silhouette width.

\subsection{Supervised 50-Gene Classifier and 9-Gene VRS Score Algorithm}
The 50-gene Random Forest classifier was built using the `randomForest` package (ntree=1,000, mtry=7) and evaluated via 10-fold cross-validation. Predictor importance was ranked by mean decrease in Gini impurity.
The continuous 9-Gene Venetoclax Response Score (VRS) is calculated as:
\begin{equation}
\text{VRS}_{\text{raw}} = \sum_{i=1}^{9} w_i \cdot z(g_i)
\end{equation}
where $g_i$ represents the 9 key genes: \textit{BCL2}, \textit{NPM1}, \textit{DNMT3A}, \textit{MCL1}, \textit{TP53}, \textit{RUNX1}, \textit{ASXL1}, \textit{CD14}, and \textit{FCGR1A}. Positive weights ($w_i = +1$) are assigned to primitive/BCL-2 addiction markers (\textit{BCL2}, \textit{NPM1}, \textit{DNMT3A}), while negative weights ($w_i = -1$) are assigned to monocytic/resistance markers (\textit{MCL1}, \textit{TP53}, \textit{RUNX1}, \textit{ASXL1}, \textit{CD14}, \textit{FCGR1A}). Raw scores are standardized via reference Z-score transformation and rescaled to a 0--100 continuous scale.

\subsection{Multivariable Variance Decomposition ($R^2$ Improvement Analysis)}
To quantify independent predictive value for drug response, hierarchical linear models were fit:
\begin{align}
\text{Model 1 (Mutational Baseline): } \text{AUC}_k &= \beta_0 + \sum_{m=1}^{11} \beta_m \cdot \text{Mutation}_m + \varepsilon \\
\text{Model 2 (Combined Model): } \text{AUC}_k &= \beta_0 + \sum_{m=1}^{11} \beta_m \cdot \text{Mutation}_m + \gamma \cdot \text{Subtype} + \varepsilon
\end{align}
The absolute variance improvement $\Delta R^2 = R^2_{\text{Model 2}} - R^2_{\text{Model 1}}$ and relative percentage improvement $\frac{\Delta R^2}{R^2_{\text{Model 1}}} \times 100\%$ were calculated. Hypothesis testing for $\gamma \neq 0$ was performed using ANOVA $F$-tests with Benjamini-Hochberg FDR adjustment.

\subsection{Single-Cell CITE-seq and DepMap Functional Essentiality Protocols}
Single-cell RNA-seq and ADT surface protein profiles from van Galen et al. \cite{vangalen2019single} ($38,412$ cells) and Pei et al. \cite{pei2020monocytic} ($N=4,426$ blasts) were processed using Seurat v4. Single-cell VRS scores were computed per cell. Cell-surface antibody-derived tags (CD34, CD117, CD11b, CD64) were normalized via CLR (Centered Log-Ratio) transformation. Cancer Dependency Map (DepMap 24Q2) Chronos gene knockout dependency scores for $24$ myeloid leukemia lines were evaluated to confirm functional target essentiality.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Supplementary/FigureS1.pdf}
\caption{\textbf{Consensus Clustering Diagnostics for Optimal $k$ Selection.} \textbf{A}, Consensus matrix heatmap at $k=2$ showing two crisp, well-separated blocks with high within-cluster consensus. \textbf{B}, Consensus cumulative distribution function (CDF) curves for $k=2$ to $k=10$. \textbf{C}, Delta area plot showing the relative change in area under the CDF curve as a function of $k$; the largest relative gain occurs at $k=2$, with diminishing incremental gains at higher $k$, supporting two clusters as the optimal partition.}
\label{fig:figS1}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Supplementary/FigureS2.pdf}
\caption{\textbf{Pediatric TARGET-AML Overall Survival Validation.} Kaplan-Meier overall survival analysis comparing molecular subtypes Cluster 1 ($N=1368$) vs Cluster 2 ($N=345$) in the pediatric TARGET-AML validation cohort, showing no significant difference ($p=0.052$).}
\label{fig:figS2}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Supplementary/FigureS3.pdf}
\caption{\textbf{Baseline Overall Survival Partitioning across ELN 2017 Subgroups.} Kaplan-Meier overall survival curves comparing Cluster 1 vs Cluster 2 within: \textbf{A}, ELN 2017 Favorable-risk cohort. \textbf{B}, ELN 2017 Intermediate-risk cohort. \textbf{C}, ELN 2017 Adverse-risk cohort. \textbf{D}, Hazard ratio forest plot across ELN risk strata.}
\label{fig:figS3}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Supplementary/FigureS4.pdf}
\caption{\textbf{Tumor Microenvironment Deconvolution and Cell Type Fraction Analysis.} Immune and stromal cell type fractions deconvoluted across BeatAML transcriptomic subtypes showing monocytic vs progenitor cell population ratios.}
\label{fig:figS4}
\end{figure}
...
\caption{\textbf{Subgroup Benchmarking against LSC17, ROC Discrimination, and Decision Curve Analysis (DCA).} \textbf{A}, Correlation between the 9-gene VRS and the prognostic LSC17 stemness score, demonstrating biological orthogonality. \textbf{B}, ROC curves comparing the discrimination accuracy of the VRS versus LSC17 for predicting ex vivo Venetoclax response. \textbf{C}, Decision Curve Analysis (DCA) showing the clinical net benefit of VRS-guided treatment selection. \textbf{D}, Bootstrap stability analysis of the ROC-AUC performance.}
\label{fig:figS5}
\end{figure}
...
\caption{\textbf{Metabolic Reprogramming, Dual Apoptotic Priming, and Pathway Integration in Monocytic Blast States.} \textbf{A}, Signature scores for Oxidative Phosphorylation (OXPHOS) and Glycolysis showing Cluster 2 as a high-energy metabolic hybrid state. \textbf{B}, Ex vivo Bliss synergy matrix for Venetoclax + S63845 (MCL-1 inhibitor) combination in primary patient blasts, showing synergy. \textbf{C}, Heatmap showing coordinated enrichment of high-energy metabolic circuits in Cluster 2.}
\label{fig:figS6}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Supplementary/FigureS7.pdf}
\caption{\textbf{Functional Genomics and DepMap CRISPR Knockout Validation of VRS Targets.} \textbf{A}, BCL2 gene expression versus CRISPR BCL2 dependency probability across cell lines. \textbf{B}, BCL2/MCL1 expression ratio versus CRISPR BCL2 dependency. \textbf{C}, CRISPR HDAC3 dependency versus VRS, demonstrating that lower VRS (resistant) aligns with greater HDAC3 vulnerability. \textbf{D}, CRISPR MAPK1 (ERK2) dependency versus VRS, showing MEK/ERK vulnerability in resistant cells.}
\label{fig:figS7}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Supplementary/FigureS8.pdf}
\caption{\textbf{Bulk Proteomic Target Validation Rescued by Functional Ratios and Tumor Purity Filtering.} \textbf{A}, Spearman correlation between bulk protein VRS (pVRS) and ex vivo Venetoclax AUC across BeatAML proteomics ($N=327$). \textbf{B}, BCL2/MCL1 protein ratio vs Venetoclax AUC. \textbf{C}, CD64/BCL2 protein ratio vs Venetoclax AUC. \textbf{D}, CD64/BCL2 protein ratio filtered for blast purity $\ge 50\%$.}
\label{fig:figS8}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Supplementary/FigureS9.pdf}
\caption{\textbf{Platform-Independent Validation of the Continuous VRS across Central European Trial Cohorts.} Boxplots of continuous 9-gene VRS stratified by molecular subtype across independent European trial cohorts (AMLCG GPL96 $N=163$, AMLCG GPL570 $N=79$, AMLSG $N=211$).}
\label{fig:figS9}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Supplementary/FigureS10.pdf}
\caption{\textbf{External Validation of the VRS and Molecular Subtypes in the BeatAML 2.0 Cohort.} \textbf{A}, Scatter plot of continuous VRS vs ex vivo Venetoclax AUC in independent BeatAML 2.0 validation cohort ($N=367$). \textbf{B}, Boxplot comparing Venetoclax response across predicted subtypes.}
\label{fig:figS10}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Supplementary/FigureS11.pdf}
\caption{\textbf{Comprehensive Baseline Prognostic Overall Survival Validation.} Kaplan-Meier overall survival curves comparing Cluster 1 vs Cluster 2 across discovery BeatAML, TCGA-LAML, and European multicenter cohorts.}
\label{fig:figS11}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Supplementary/FigureS12.pdf}
\caption{\textbf{Immunophenotypic Coupling of Single-Cell Resistance Hierarchy with ADT Cell Surface Markers.} Single-cell CITE-seq ADT surface protein distributions (CD34, CD117, CD11b, CD64) across single-cell VRS tiers.}
\label{fig:figS12}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Supplementary/FigureS13.pdf}
\caption{\textbf{Longitudinal Selection Dynamics of Single-Cell VRS and Cell Surface ADT Markers under Venetoclax Combination Therapy.} Violin plots comparing Diagnosis vs Relapse from matched longitudinal patient CITE-seq profiles under Venetoclax + Decitabine therapy.}
\label{fig:figS13}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Supplementary/FigureS14.pdf}
\caption{\textbf{UMAP Visualization of Single-Cell VRS and Cell Surface ADT Markers in GSE143363 CITE-seq Profiles.} Single-cell UMAP projections colored by: \textbf{A}, Clinical timepoint (Diagnosis vs Relapse). \textbf{B}, Single-cell VRS. \textbf{C}, CD34 surface protein expression. \textbf{D}, CD64 surface protein expression.}
\label{fig:figS14}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Supplementary/FigureS15.pdf}
\caption{\textbf{Validation of the Monocytic Resistance Axis and Age-Independence of the VRS.} \textbf{A}, Monocytic differentiation scores between Cluster 1 and Cluster 2. \textbf{B}, VIALE-A trial resistance signature score between clusters. \textbf{C}, Ex vivo Venetoclax response against patient age.}
\label{fig:figS15}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Supplementary/FigureS16.pdf}
\caption{\textbf{Cox Proportional Hazards Model Diagnostics and Subgroup Audits.} \textbf{A}, Forest plot of subtype hazard ratio consistency across clinical and mutational subgroups. \textbf{B}, Schoenfeld residuals test for checking the proportional hazards assumption. \textbf{C}, Calibration plot comparing observed versus predicted 3-year survival. \textbf{D}, Restricted Mean Survival Time (RMST) difference analysis between subtypes.}
\label{fig:figS16}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Supplementary/FigureS17.pdf}
\caption{\textbf{Benchmarking of the 9-Gene VRS against ELN 2017/2022 Risk Groups.} \textbf{A}, Boxplot of continuous VRS across ELN risk categories. \textbf{B}, Scatter plots of continuous VRS against ex vivo Venetoclax response (AUC) within each ELN risk category.}
\label{fig:figS17}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Supplementary/FigureS18.pdf}
\caption{\textbf{Single-Cell Resolution Validation of the 9-Gene VRS and Developmental Partitioning.} Single-cell RNA-seq validation in bone marrow hierarchy (van Galen dataset, GSE116256): \textbf{A}, UMAP of cell types. \textbf{B}, VRS projection across cells. \textbf{C}, VRS by lineage. \textbf{D}, BCL2 vs MCL1 trade-off.}
\label{fig:figS18}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Supplementary/FigureS19.pdf}
\caption{\textbf{VRS Robustness and Technical Noise Sensitivity Analysis.} Sensitivity analysis demonstrating classification concordance and Pearson correlation under simulated technical noise levels up to 50\% of the true standard deviation.}
\label{fig:figS19}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Supplementary/FigureS20.pdf}
\caption{\textbf{Classifier Confidence Distribution and Interactive VRS Web Calculator.} Poster mapping confidence distribution across external validation cohorts and Streamlit interactive bedside VRS web calculator interface.}
\label{fig:figS20}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Supplementary/FigureS21.pdf}
\caption{\textbf{Technical Batch Mitigation and Batch Effect Correction.} \textbf{A}, Principal Component Analysis (PCA) of raw gene expression data showing clear batch effects across sequencing centers. \textbf{B}, PCA of ComBat-seq corrected expression data showing removal of technical batch effects. \textbf{C}, Expression levels of top 10 batch-associated genes before correction. \textbf{D}, Expression levels of the same 10 genes after ComBat-seq correction, demonstrating that technical variation is successfully removed.}
\label{fig:figS21}
\end{figure}
