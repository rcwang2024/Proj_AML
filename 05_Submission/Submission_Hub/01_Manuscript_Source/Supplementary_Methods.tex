...
\caption{\textbf{Technical Batch Mitigation and Consensus Clustering Cumulative Distribution Function (CDF).} (A) Empirical Bayes batch correction (ComBat) across sequencing centers. (B) Consensus CDF curves across cluster counts ($k=2$ to $k=6$). (C) Relative change in area under CDF curve. (D) Silhouette width distribution for $k=2$ consensus subtypes.}
\label{fig:figS1}
\end{figure}
...
\caption{\textbf{Cox Proportional Hazards Model Diagnostic Audits.} Schoenfeld residual diagnostic plots for Cox proportional hazards model variables in the primary BeatAML cohort.}
\label{fig:figS2}
\end{figure}
...
\caption{\textbf{Subgroup Benchmarking and Decision Curve Analysis (DCA).} (A) Forest plot of subtype predictive performance across mutation subgroups. (B) Decision Curve Analysis (DCA) evaluating net benefit of VRS-guided treatment selection across clinical decision thresholds.}
\label{fig:figS5}
\end{figure}
...
\caption{\textbf{Metabolic Reprogramming and Dual Apoptotic Priming in Monocytic Blast States.} (A) Gene Set Enrichment Analysis (GSEA) of Oxidative Phosphorylation (OXPHOS) vs Glycolysis pathways in Cluster 1 vs Cluster 2. (B) Ex vivo Bliss synergy matrix for Venetoclax + S63845 (MCL-1 inhibitor) combination in primary patient blasts.}
\label{fig:figS6}
\end{figure}
...
\caption{\textbf{Supervised 50-Gene Lineage Classifier Feature Importance and Performance.} (A) Gini importance ranking of top 50 classifier features. (B) 10-fold cross-validation ROC curve for subtype assignment ($AUC = 0.994$).}
\label{fig:figS7}
\end{figure}
...
\caption{\textbf{DepMap Functional Essentiality Screening across Myeloid Leukemia Lines.} Chronos knockout dependency scores for BCL2, MCL1, and salvage target genes across 24 myeloid leukemia cell lines.}
\label{fig:figS16}
\end{figure}
...
\caption{\textbf{GSE106291 Chemotherapy Induction Remission ROC Curve (Target-Specific Negative Control).} ROC curve of 9-gene VRS predicting clinical complete remission under intensive 7+3 chemotherapy induction ($ROC-AUC = 0.550, p = 0.260$).}
\label{fig:figS19}
\end{figure}
