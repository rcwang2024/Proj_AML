...
\end{equation}
where $g_i \in \{\text{BCL2}, \text{NPM1}, \text{DNMT3A}, \text{MCL1}, \text{TP53}, \text{RUNX1}, \text{ASXL1}, \text{CD14}, \text{FCGR1A}\}$, with positive weights $w_i = +1$ for primitive/BCL-2 addiction markers ($\text{BCL2}, \text{NPM1}, \text{DNMT3A}$) and negative weights $w_i = -1$ for monocytic/resistance markers ($\text{MCL1}, \text{TP53}, \text{RUNX1}, \text{ASXL1}, \text{CD14}, \text{FCGR1A}$). Raw scores are standardized via reference Z-score transformation and rescaled to a 0--100 continuous scale.
